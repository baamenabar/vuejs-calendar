...
export default {
	...
	computed: {
        ...
        days() {
            ...
            let firstWeekDay = days[0].day() - 1;
            firstWeekDay = firstWeekDay < 0 ? (7 + firstWeekDay) : firstWeekDay;
            const prevMonthDays = Array.from( 
                Array(firstWeekDay).keys(), 
                (weekDayNumber) => {
                	return this.$moment(days[0]).subtract(weekDayNumber + 1, 'days') 
                }
            ).reverse();
